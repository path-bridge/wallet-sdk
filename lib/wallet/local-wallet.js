...
class LocalWallet {
    constructor(wif, addressType = types_1.AddressType.P2WPKH, network = bitcoin_core_1.bitcoin.networks.bitcoin) {
        ...
        this.scriptPk = (0, address_1.publicKeyToScriptPk)(this.pubkey, addressType, network);
    }
    static fromMnemonic(addressType, network, mnemonic, passPhrase, hdPath) {
        const keyring = new keyring_1.HdKeyring({ mnemonic, hdPath, passphrase: passPhrase });
        const keyPair = keyring.getAccounts()[0];
        const wallet = new LocalWallet(keyPair.privateKey.toString("hex"), addressType, network);
        return wallet;
    }
