 ...
 */
function signMessageOfBIP322Simple({ message, address, network, wallet, }) {
    const outputScript = (0, address_1.addressToScriptPk)(address, network);
    const addressType = (0, address_1.getAddressType)(address, network);
...
function verifyMessageOfBIP322Simple(address, msg, signature, network = bitcoin_core_1.bitcoin.networks.bitcoin) {
    const addressType = (0, address_1.getAddressType)(address, network);
    if (addressType === types_1.AddressType.P2WPKH ||
        addressType === types_1.AddressType.M44_P2WPKH) {
        return verifySignatureOfBIP322Simple_P2PWPKH(address, msg, signature, network);
    }
    else if (addressType === types_1.AddressType.P2TR ||
        addressType === types_1.AddressType.M44_P2TR) {
        return verifySignatureOfBIP322Simple_P2TR(address, msg, signature, network);
    }
    return false;
}
exports.verifyMessageOfBIP322Simple = verifyMessageOfBIP322Simple;
function verifySignatureOfBIP322Simple_P2TR(address, msg, sign, network = bitcoin_core_1.bitcoin.networks.bitcoin) {
...
function verifySignatureOfBIP322Simple_P2PWPKH(address, msg, sign, network = bitcoin_core_1.bitcoin.networks.bitcoin) {
