 ...
 */
function publicKeyToPayment(publicKey, type, network) {
 ...
 */
function publicKeyToScriptPk(publicKey, type, network) {
    const payment = publicKeyToPayment(publicKey, type, network);
 ...
 */
function isValidAddress(address, network = bitcoin_core_1.bitcoin.networks.bitcoin) {
    let error;
    try {
        bitcoin_core_1.bitcoin.address.toOutputScript(address, network);
    }
    catch (e) {
        error = e;
    }
 ...
 * Get address type.
 */
function getAddressType(address, network = bitcoin_core_1.bitcoin.networks.bitcoin) {
    let type;
    try {
 ...
 */
function scriptPkToAddress(scriptPk, network = bitcoin_core_1.bitcoin.networks.bitcoin) {
    try {
