 ...
 */
class Transaction {
    constructor(network, feeRate, changedAddress, enableRBF = true) {
        this.utxos = [];
        this.inputs = [];
        this.outputs = [];
    ...
    createEstimatePsbt() {
        const estimateWallet = wallet_1.EstimateWallet.fromRandom(this.inputs[0].utxo.addressType, this.network);
        const scriptPk = (0, address_1.addressToScriptPk)(estimateWallet.address, this.network).toString("hex");
        const tx = this.clone();
        tx.utxos.forEach((v) => {
