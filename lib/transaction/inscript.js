...
function estimateRevealTxSize(network, publicKey, commitTxData, toAddress, amount) {
...
function buildCommitTx(network, publicKey, commitTxDatas, inscriptions, unspents, changeAddress, feeRate) {
    const tx = new transaction_1.Transaction(network, feeRate, changeAddress);
    // const network = toPsbtNetwork(networkType);
    let totalOutAmount = 0;
    for (var i = 0; i < commitTxDatas.length; i++) {
        const outputAmount = estimateRevealTxSize(network, publicKey, commitTxDatas[i], inscriptions[i].revealAddr, constants_1.UTXO_DUST) * feeRate + constants_1.UTXO_DUST;
...
function buildRevealTx(network, commitTxData, inscription, commitTx, index = 0) {
    const { scriptTaproot, tapLeafScript } = commitTxData;
    const psbt = new bitcoin_core_1.bitcoin.Psbt({ network });
    psbt.addInput({
        hash: commitTx.getId(),
