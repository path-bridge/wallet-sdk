...
describe("verifyMessage", function () {
  it("ecdsa", async function () {
    ...
    const signature = wallet.signMessage(message, "ecdsa");
    const pubkey = wallet.getPublicKey();
    const result = verifyMessageOfECDSA(pubkey, message, signature);
    expect(result).eq(true);

    const errorResult = verifyMessageOfECDSA(
      pubkey,
      message,
  ...
  it("bip322-simple for P2WPKH", async function () {
    ...
    const signature = wallet.signMessage(message, "bip322-simple");
